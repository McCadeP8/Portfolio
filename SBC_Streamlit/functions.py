...
def overall_cap_table(df: pd.DataFrame, exceptions_df: pd.DataFrame, base_cap: pd.DataFrame) -> pd.DataFrame:
    df = pd.DataFrame({
        ...
        "Cap Space": [int(round(current_salary_cap - get_cap_total(df, exceptions_df, team), 0)) for team in team_info.keys()],
        "Tax Space": [int(round(current_luxury_tax - get_tax_total(df, team), 0)) for team in team_info.keys()],
        "Hard Cap": [team_hard_cap(base_cap, team) for team in team_info.keys()],
        "Apron 1 Space": [int(round(current_apron_1 - get_tax_total(df, team), 0)) for team in team_info.keys()],
        "Apron 2 Space": [int(round(current_apron_2 - get_tax_total(df, team), 0)) for team in team_info.keys()],
